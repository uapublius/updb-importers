...
export function isDistrictOf(country: string = '', district: string = '') {
  ...
  return provincesUpperCased.some(p => (p.name === district || p.short === district || p.alt?.includes(district)) && p.country === country);
}
...
export let getCountryAbbreviationByName = (name: string = '') => {
  ...
  let country = Object.entries(countries).find(([k, c]) => c.name?.toUpperCase() === name || k?.toUpperCase() === name);
