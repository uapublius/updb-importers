...
export function fileToRecord(filename: string, file: string) {
  ...
  let record = window.document.querySelector('body > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td');
  let record2 = window.document.querySelector('body > center > table > tbody > tr:nth-child(2) > td > font');
  let record3 = window.document.querySelector('body > center > table > tbody > tr:nth-child(2) > td');
    ...
    if (el.nodeName === "A") {
      ...
      if (matches?.length > 1) {
        ...
        if (!skip.some(s => href.includes(s))) {
          console.warn("Unknown link found:", href);
        }
      }
    }
...
export function recordToReport(record: UfoDnaRecord, file: string, referencesMap: Record<string, number>): FullRecord {
...
function buildReferences(record: UfoDnaRecord, referencesMap: Record<string, number>, file: string) {
  let references: Reference[] = record.references.filter(r => referencesMap[r]).map(r => ({ id: referencesMap[r] }));
...
export function buildDate(record: UfoDnaRecord) {
  ...
  if (!parsed?.length) {
    ...
    if (year?.length) {
      ...
      throw new Error('Cannot parse date: ' + eventDate);
    }
  }
