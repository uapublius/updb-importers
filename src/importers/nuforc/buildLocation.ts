import { cleanText, getCountryAbbreviationByName, isDistrictOf } from '../../utils';
...
export function buildLocation(record: NuforcRecord) {
  let locationRecord = record.location.toUpperCase().match(/(.*)?\,(.*)/).map(s => s?.trim());
...
export function buildLocationLength3(locationRecord: string[], record: NuforcRecord) {
  ...
  if (isSameDistrictAndCountry)
    return buildLocationLength2(locationRecord);
